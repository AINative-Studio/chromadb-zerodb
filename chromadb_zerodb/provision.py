...
def auto_provision(base_url: str = ZERODB_API) -> tuple:
    ...
    # 3. Auto-provision
    print("[chromadb-zerodb] Auto-provisioning free ZeroDB project...", file=sys.stderr)
    try:
        resp = requests.post(
            f"{base_url}/api/v1/zerodb/projects/provision",
            json={"source": "chromadb-zerodb"},
            timeout=30,
        )
    ...
    print(
        f"[chromadb-zerodb] Project provisioned! Claim it at: {claim_url}",
        file=sys.stderr,
    )
    return api_key, project_id
